...
if (what != null && what.equalsIgnoreCase(USR_CODE)) {
	String userName, usrPasswd, roles="", grps="", 
					prjs="", lastName, firstName, email;
	Integer usrId;
	Integer disable;
	...
	if (frmId != null) {
		...
		if (theUsr != null) {
			...
			email = (theUsr.getEmail() == null)? "": theUsr.getEmail();
			disable = theUsr.getRemoved();
			...
			msg = "";
			jsonStr = "{\"res\":"+1+",\"msg\":\""+msg+"\",\"id\":"+usrId+",\"email\":\""+email;
