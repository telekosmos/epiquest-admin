...
var KEY_UP = 38,
	...
	KEY_ESC = 27,
	KEY_F4 = 115,
	IMG_LOCATION = APP_ROOT+'/web/img';


// As it is supossed not to change the font size, we assume 10px wide per char
var FONT_CHAR_SIZE = 7.5;


$.widget('ui.combobox', {
// This is a field to hold the variable length of the options holder
// This one allows for the compenent hold every option in one line
	holderWidth: 0,
...
// "enhancement": now every option fill just one line
		var maxLength = 0;
		var myOptions = null;
		myOptions = (this.element[0].tagName.toLowerCase() == 'select')? 
						this.element[0].options:
						options.data;
		
		$.each(myOptions, function(i, val) {	
				var opt;
				if (val.nodeName)
					opt = myOptions[i].text;
				else
					opt = myOptions[i];
				
				maxLength = (opt.length > maxLength)? opt.length: maxLength;
			});
		this.holderWidth = maxLength * FONT_CHAR_SIZE;
...
//		styles.width = this.element.width() + this.holderWidth;
		styles.width = this.holderWidth;
...
//		arrowURL: 'drop_down.png',
		arrowURL: IMG_LOCATION+'/drop_down.png',
		arrowHTML: function() {
...
/**
 * Fills the options member of this element with values
