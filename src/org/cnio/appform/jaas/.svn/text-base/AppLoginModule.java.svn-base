...
System.out.println("loginModule.login: after no success (errCause: "+errCause+")");
				hibSes.close();
			} // EO !success
			...
			success = false;
System.err.println("AppLoginModule exception: ");
ex.printStackTrace();
...
//			ctrl.logSessionInit(user.getId(), user.getUsername(), "", "", "",
	//				AppUserCtrl.LOGIN_CONCURRENT);
		...
		Transaction tx = null;
//		passwdOk = passwd.equals(user.getPasswd());
		passwdOk = auv.valUser(username, passwd);
		if (passwdOk) {
			String strRoles = "";
			...
			AppPrincipal userPpal = 
							new AppPrincipal (user.getFirstName(), user.getLastName(), username);
			userPpal.setLoggedIn(true);
			tempPrincipals.add(userPpal);

			
			List<Role> roles = ctrl.getRoleFromUser(user);
			for (Role role: roles) {
				tempPrincipals.add(new AppPrincipalRole (role.getName()));
...
/* System.out.println("LoginModule.loginValidate: "+strRoles);
			try {
				...
				LogFile.error("Unable to set user loggedIn in the database");
				StackTraceElement[] stack = exHib.getStackTrace();
...
*/
			...
			try {
				errCause = AppUserCtrl.LOGIN_MISMATCH_PASSWD;
				...
				if (numAttempts.compareTo(HibernateUtil.MAX_LOGIN_ATTEMPTS) == 0) {
					...
					user.setLoginAttempts(++numAttempts);
				}
