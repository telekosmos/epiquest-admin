 ...
 */	
	 ...
	 * @param sec, the section whose items are gonna be retrieved
	 */	
		public void viewSection (Session hibSes, Section sec) {
	// i get all items for the section
	// ii if the item is a text, print the text
	/* iii if the item is a question
	 * iv print the content
	 * v check if it is a grouping question
	 * vi if so, get the items for the question and go back ii
	 ...
	 */
			...
			Collection<AbstractItem> items = 
															HibernateUtil.getContainers4Section(hibSes, sec);
...
LogFile.display("num of containers in section: "+items.size());			
		...
		private void initLog () throws java.io.IOException {
			...
			Appender myAppender = new FileAppender(myLayout, "appform.log");    // Appender is
			                                               // Interface
 ...
 *  
 * @param highlight, the value to translate the highlight
 */		
		...
		/**
		 * Print the html code for an item, considering its children questions if
		 * so
		 * @param hibSes
		 * @param ai, the item (and its children) which is gonna be rendered
		 * @param patId, the patient id (necessary to get the answers)
		 * @param intrCtrl, the controller of the interview to get the answers
		 */
		public void html4Item (Session hibSes, AbstractItem ai, 
						Integer patId, IntrvFormCtrl intrCtrl) {
...
//				addHtmlStr("<table bgcolor=\"lightgray\" id=\"t"+ai.getId()+"-children\">");
				...
				if (ai.getRepeatable() != null && ai.getRepeatable() == 1) {
					List<AbstractItem> containees = 
						HibController.ItemManager.getOrderedContainees(hibSes, ai);
					
					renderChildren (hibSes, intrCtrl, ai, containees, patId);
					addHtmlStr(ETRTD+"<tr><td align=\"right\"><input type=\"button\" value=\" + \"" +
							" name=\"btn-t"+ai.getId()+"\" id=\"btn-t"+ai.getId()+"\"" +
									"	onclick = \"ctrl.addElem('t"+ai.getId()+"')\"></td></tr>");
					addHtmlStr("</table>");
					
					return;
				}
				else
					addHtmlStr("<table bgcolor=\"#FFCC99\" id=\"t"+ai.getId()+"-children\">");
			}
	 ...
	 * @param numOfAnswers, the numOf answers (necessary for the multiple answers
	 * case)
	 * @param patId, the patient id
	 * @param intrCtrl, the controller of the interview
	 */	
		private void renderAnswersPat (Session hibSes, AbstractItem ai, int numOfAnswers, 
															Integer patId, IntrvFormCtrl intrCtrl) {
			int numAns = 1;
			
			List<Object[]> answers = intrCtrl.getAnswers((Question) ai, patId, numAns);
...
// Concurrent changes are not allowed						
					...
					inputVal = (inputVal.equalsIgnoreCase(RenderEng.MISSING_ANSWER))? "":
										inputVal;
					numQuestion = (answer != null) ? Integer
							.toString(((Integer) answer[2]).intValue()) : "1";

					if (ansi instanceof AnswerType) { // simple answer (label, number, decimal)
...
// and the last 1 is the answer order besides, we gotta check the value...
...
*/						
// Add the onblur event depending on the datatype and the realtime switch
...
*/
						...
						addHtmlStr("ERROR: Not implemented case -&gt: "
								+ StringEscapeUtils.escapeHtml(ansi.getName())
								+ "<br>");
					}
				...
				if (((Question) ai).getRepeatable().intValue() == 1
					...
					String plusBtn = "<input type=\"button\" name=\"btn-q" +
						ai.getId() + "-" + numQuestion + "-" + orderCai +"\" id=\"btn-q" +
						ai.getId() + "-" + numQuestion + "-" + orderCai +"\" ";
				...
				numAns++;
				answers = intrCtrl.getAnswers((Question) ai, patId, numAns);
			} // while items...
	...
	/**
	 * Renders a question which doesent still have any answer (which implies nothing
	 * about it is in the database)
	 * @param hibSes, the hibernate session
	 * @param ai, the question itself
	 */	
		private void renderQuestion (Session hibSes, AbstractItem ai) {

			List<AnswerItem> cai = HibernateUtil.getAnswerTypes4Question(
					hibSes, ai);
...
/*						
						addHtmlStr (" onblur=\"intrvFormCtrl.chkDecimal(this '"+
										ranges[0]+"', '"+ranges[1]+"', '"+ranges[2]+"')\"");
						addHtmlStr (" onblur=\"intrvFormCtrl.sendItem('D', this '"+
								ranges[0]+"', '"+ranges[1]+"', '"+ranges[2]+"')\"");
*/
					}
					else // normal text
...
// System.out.println("renderQuestion: realtime="+realtime);
					if (realtime == 0)  {
						if (datatype.equalsIgnoreCase("L"))
							addHtmlStr (" onblur=\"event.stopPropagation();intrvFormCtrl.onBlur(this.name)\"");
						else
...
// System.out.println(htmlStr);			
	 ...
	 * @param patId
	 */	
		private void renderChildren (Session hibSes, IntrvFormCtrl intrCtrl, AbstractItem parent, 
																List<AbstractItem> containees, Integer patId) {
			Integer ansNum = 1;
			boolean end = false;
			int numOfAnswers = 1;
	// hay que iterar sobre los containees y, 
	// por fuera, sobre el número de preguntas
	// Hay que poner la tabla al principio de cada containee pero no al final
	// => sobra una tabla
			...
			while (!end) {
				...
				for (int i=0; i<containees.size(); i++) {
					...
					if (i == 0) {
						String newName = "<table bgcolor=\"#FFCC99\" id=\"t"+parent.getId()+"-children\"";
	...
	// this call gets the answers for a question regarding to teh answer number (ansNUm)					
						List<Object[]> answers = intrCtrl.getAnswers ((Question)child, patId, ansNum);

						List<AnswerItem> anstypes = 
									HibernateUtil.getAnswerTypes4Question(HibernateUtil.getSessionFactory().getCurrentSession(), child);
...
/*						addHtmlStr("<tr><td align=\"right\">");
						addHtmlStr("<input type=\"button\" name=\"btn-t"+
									...
									parent.getId()+"\" onclick=\"ctrl.addElem('t"+parent.getId()+"')\">");
						addHtmlStr(ETRTD);
...
*/						return;
			...
			} // while answers iteration is over for every containee
		...
		private void renderContaineeItem (Session hibSes, Question q, List<Object[]> answers, 
				List<AnswerItem> anstypes) {
			...
			String numAnswer;
			List<QuestionsAnsItems> lQai = 
										HibController.ItemManager.getPatterns(hibSes, q);
			addHtmlStr(ITRTD);
...
System.out.println("renderContaineeItem: realtime="+realtime);
					if (realtime == 0)  {
						if (datatype.equalsIgnoreCase("L"))
							addHtmlStr (" onblur=\"event.stopPropagation();intrvFormCtrl.onBlur(this.name)\"");
						else
					...
					String sel = "selected=\"selected\"";
					List<EnumItem> ceni = 
						HibController.EnumTypeCtrl.getEnumItems(HibernateUtil.getSessionFactory().getCurrentSession(),(EnumType) ansi);
					
					String selName = "q" + q.getId() + "-" + numAnswer + "-" +	orderCai,
								htmlPiece;
...
//				addHtmlStr("\nEnd of renderItem");
		...
		private void renderSingleItem (Session hibSes, AbstractItem ai, 
											Integer patId, IntrvFormCtrl intrCtrl) {
	 ...
	 * Renders an item as its answer_item says. This method works in a recursive 
	 * way to be able to render all items inside one item (superquestions)
	 * @param hibSes, the session to perform the transaction
	 * @param item, the item which is gonna be rendered
	 */
	...
	// first render the answer items			
				Collection<AnswerItem> ansItems = 
									HibernateUtil.getAnswerTypes4Question(hibSes, item);			
				if (ansItems != null && !ansItems.isEmpty()) {
					...
					while (itType.hasNext()) {
						...
						if (ansIt instanceof AnswerType) 
							...
							Collection<EnumItem> colEnums = 
												HibController.EnumTypeCtrl.getEnumItems(hibSes, (EnumType)ansIt);
			...
			Collection<AbstractItem> items = 
									HibController.ItemManager.getOrderedContainees(hibSes, item);
// LogFile.stderr("num of containees for " +item.getId()+": "+items.size());			
			if (items != null && !items.isEmpty()) {				
LogFile.display("[This is a SuperQuestion with "+items.size()+" items]");					
				Iterator<AbstractItem> itItem = items.iterator();
				while (itItem.hasNext())
